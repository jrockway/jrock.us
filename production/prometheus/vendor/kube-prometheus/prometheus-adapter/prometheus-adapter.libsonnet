  ...
  _config+:: {
    ...
    prometheusAdapter+:: {
      ...
      config: |||
        resourceRules:
          cpu:
            containerQuery: sum(rate(container_cpu_usage_seconds_total{<<.LabelMatchers>>,pod!=""}[5m])) by (<<.GroupBy>>)
          ...
          memory:
            containerQuery: sum(container_memory_working_set_bytes{<<.LabelMatchers>>,pod!=""}) by (<<.GroupBy>>)
